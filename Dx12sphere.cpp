...
struct scene_constants {
    ...
    glm::mat4 light_view_proj; // for shadow mapping
    ...
    glm::vec4 ambient_color2;
    glm::vec4 diffuse_color2;
    glm::vec4 specular_color2;
...
int main(int, char**)
{
    ...
    // Setup matrices and camera using GLM
    // Compute view and projection matrices then transpose for HLSL
    glm::mat4 view_m = glm::lookAt(glm::vec3(0.f, 0.f, -10.f),
        glm::vec3(0.f, 0.f, 0.f),
        glm::vec3(0.f, 1.f, 0.f));
    glm::mat4 proj_m = glm::perspective(glm::radians(45.0f),
    ...
    glm::mat4 light_view_proj = lightProj * lightView;

    // Store transposed matrices in constants (matching original behavior)
    scene_constants constants = {};
    constants.model_matrix = glm::transpose(glm::mat4(1.0f));
    constants.view_matrix = glm::transpose(view_m);
    constants.proj_matrix = glm::transpose(proj_m);
    constants.light_view_proj = glm::transpose(light_view_proj);
    constants.light_position = glm::vec4(0.f, 5.f, 0.f, 1.f);
    constants.light2_position = glm::vec4(0.f, 5.f, 0.f, 1.f);
    constants.camera_position = glm::vec4(0.f, 0.f, -10.f, 1.f);
    constants.ambient_color = glm::vec4(0.5f);
    constants.diffuse_color = glm::vec4(1.f);
    constants.specular_color = glm::vec4(1.f);
    constants.specular_power = 32.f;
    constants.ambient_color2 = glm::vec4(0.5f);
    constants.diffuse_color2 = glm::vec4(1.f);
    constants.specular_color2 = glm::vec4(1.f);
    ...
    while (running)
    {
        ...
        constants.light_position = glm::vec4(5.0f * cosf(time), 5.f, 5.0f * sinf(time), 1.f);
        constants.light2_position = glm::vec4(5.0f * cosf(-time), 5.f, 5.0f * sinf(-time), 1.f);
        constants.diffuse_color = glm::vec4(r_color, g_color, b_color, 1.f);
        constants.specular_color = glm::vec4(r_color, g_color, b_color, 1.f);
        constants.ambient_color = glm::vec4(0.5f * r_color, 0.5f * g_color, 0.5f * b_color, 1.f);
        constants.diffuse_color2 = glm::vec4(r2_color, g2_color, b2_color, 1.f);
        constants.specular_color2 = glm::vec4(r2_color, g2_color, b2_color, 1.f);
        constants.ambient_color2 = glm::vec4(0.5f * r2_color, 0.5f * g2_color, 0.5f * b2_color, 1.f);

        // Update sphere model with unpredictable motion and transpose the resulting matrix
        ...
        glm::mat4 translation = glm::translate(glm::mat4(1.0f), glm::vec3(randomX, randomY, randomZ));
        glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), time, glm::vec3(0.f, 1.f, 0.f));
        constants.model_matrix = glm::transpose(rotation * translation);

        // light_view_proj remains unchanged for now
        constants.light_view_proj = glm::transpose(light_view_proj);
